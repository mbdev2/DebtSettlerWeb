...
const AddItemModal = ({ modalState, closeModal }) => {
  ...
  const addItem = async (data) => {
    ...
    console.log(newData);
    try {
      const res = await axios.post("/api/seznam/novo", newData, {
        headers: {
      ...
      <Dialog open={modalState} onClose={handleModalClose}>
        ...
        <div className={styles.modalWrapper}>
          <div className={styles.anotherWrapper}>
            ...
            >
              <h2 className={styles.heading}>Add a new item</h2>
              <form className={styles.form} onSubmit={handleSubmit(addItem)}>
